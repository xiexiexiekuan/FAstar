...
void scoring::ScoreCache::read(std::string filename) {
    ...
    while (!in.eof()) {
        ...
        if(tokens[0] == "num_records") {
            ...
            numRecord = std::atoi(tempNum.c_str());
        }
    ...
    modulus = numRecord / 1000 + 0.5; //0.5 is used to correct the threshold coefficient of 0
    //modulus = 0.5;

    //The modulus can be set according to the number of variables during the test time
    // if(variableCount >= 25) modulus = 0.09;
    // else if(variableCount > 20)modulus = 0.4-(variableCount-20)*0.05;
    //     else if(variableCount == 20) modulus = 2;

    //Set the modulus to 0 when testing memory
    //modulus = 0.0; 
